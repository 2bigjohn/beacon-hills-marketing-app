...
const DEFAULT_SETTINGS = { restaurantName:"Beacon Hills", foodTarget:29, bevTarget:22, wasteTarget:2, apiKey:"" };
...
// ─── AI call ─────────────────────────────────────────────────────────────────
async function aiCall(messages, maxTokens=4000, apiKey="") {
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter',system-ui,sans-serif",paddingBottom:90}}>
      <style>{`*{box-sizing:border-box}input,select{outline:none}input:focus,select:focus{border-color:#f0a500!important}::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-thumb{background:#2e3248;border-radius:4px}@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&family=JetBrains+Mono:wght@400;600;700&display=swap');`}</style>
...
// ─── SCAN TAB ────────────────────────────────────────────────────────────────
function ScanTab({items,setItems,walks,setWalks,scans,setScans,show,settings}) {
...
function LiquorTab({liquor,setLiquor,show,bevSales,setBevSales,lqTotals,settings,canFinance}) {
  ...
  const processFile=async file=>{
    ...
    try{
      ...
      const parsed=await aiCall(msgs, 4000, settings?.apiKey);
...
// ─── PURCHASES TAB ───────────────────────────────────────────────────────────
function PurchasesTab({purchases,setPurch,items,setItems,liquor,setLiquor,priceHist,setPH,show,settings}) {
  ...
  const processInvoice=async file=>{
    ...
    try{
      ...
      else{const isImg=file.type.startsWith("image/");setProgress({stage:"Encoding file…",pct:30,sub:"Preparing image"});const d64=await b64(file);setProgress({stage:"Sending to AI…",pct:50,sub:"Reading invoice"});msgs=[{role:"user",content:[{type:isImg?"image":"document",source:{type:"base64",media_type:file.type,data:d64}},{type:"text",text:`Parse vendor invoice for Beacon Hills.\nKnown items: ${names}\nSchema: ${schema}\n${rules}`}]}];}
      setProgress({stage:"AI parsing invoice…",pct:70,sub:"Extracting line items"});
      const p=await aiCall(msgs, 4000, settings?.apiKey);
...
// ─── PRICES TAB ──────────────────────────────────────────────────────────────
function PricesTab({items,setItems,liquor,setLiquor,priceHist,setPH,show,walks,setWalks,settings}) {
  const [loading,setLoad]=useState(false); const [progress,setProgress]=useState(null); const [error,setErr]=useState("");
  ...
  const process=async file=>{
    ...
    try{
      ...
      else{const isImg=file.type.startsWith("image/");setProgress({stage:"Encoding file…",pct:30,sub:"Preparing image"});const d64=await b64(file);setProgress({stage:"Sending to AI…",pct:50,sub:"Reading cost report"});msgs=[{role:"user",content:[{type:isImg?"image":"document",source:{type:"base64",media_type:file.type,data:d64}},{type:"text",text:`Parse price/cost report.\nKnown items: ${names}\nReturn ONLY JSON:\n{"prices":[{"name":"","unitCost":0,"unit":"lb","matchExisting":false,"existingName":""}],"reportDate":"YYYY-MM-DD","notes":""}`}]}];}
...
function WasteTab({waste,setWaste,items,liquor,purchases,show,settings,canFinance}) {
  ...
  const save=()=>{const qty=parseFloat(form.qty)||0;if(!form.itemId||qty<=0)return;const item=all.find(i=>i.id===form.itemId);setWaste(p=>[{id:uid(),...form,qty,cost:parseFloat(form.cost)||est,itemName:item?.name||"",category:item?.category||""},...p]);setForm(blank);setSF(false);show("Waste logged");};
      ...
      {recipe.ingredients.map(ing=>{
        ...
        return(<div key={ing.id} style={{padding:"10px 14px",borderBottom:`1px solid ${C.border}15`}}>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr auto",gap:6,alignItems:"center",marginBottom:4}}>
            <select style={{...S.inp,fontSize:11}} value={ing.itemId} onChange={e=>updIng(recipe.id,ing.id,{itemId:e.target.value})}><option value="">-- Select --</option><optgroup label="Food">{items.map(i=><option key={i.id} value={i.id}>{i.name}</option>)}</optgroup><optgroup label="Bar">{liquor.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</optgroup></select>
  ...
  return(<>
    <div style={S.card}><div style={S.hd}><span style={S.title()}>Restaurant</span></div>
      <div style={{padding:14,display:"grid",gap:10}}>
        ...
        <div>
          <div style={S.lbl}>ANTHROPIC API KEY <span style={{color:C.red}}>*required for AI features</span></div>
          <input style={{...S.inp,marginTop:4,fontFamily:mono,letterSpacing:settings.apiKey?"2px":"normal"}} type="password" value={settings.apiKey||""} onChange={e=>upd("apiKey",e.target.value)} placeholder="sk-ant-…"/>
          {!settings.apiKey&&<div style={{fontFamily:mono,fontSize:9,color:C.amber,marginTop:4,letterSpacing:1}}>Add key to enable scan, invoice parsing, and price import.</div>}
          {settings.apiKey&&<div style={{fontFamily:mono,fontSize:9,color:C.green,marginTop:4,letterSpacing:1}}>✓ API key set — AI features enabled.</div>}
        </div>
      </div>
    </div>
